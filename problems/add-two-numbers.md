...
|:---:|:---:|:---:|:---:|
| javascript  | 112 ms | N/A | 2018/11/10 9:53 |
...
记住要写一个进位器，然后因为进位器是从个位开始的，所以就不用倒过来了。如果用JavaScript内置的数组倒序，那么会需要很长时间，且大数会出现运算精度损失的问题。

可以使用fakeHead作为开始，fakeHead下一个节点作为返回的链表的第一个节点。
